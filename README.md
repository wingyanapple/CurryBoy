...
3. 撳 **Create new project**，等 1–2 分鐘整緊個資料庫

## 第 2 步：貼 SQL 建立資料表

1. 左邊菜單揀 **SQL Editor** → **New query**
2. 打開本專案嘅 **`supabase_setup.sql`**，**全部內容複製**，貼入去
3. 撳右下角 **Run**（或 ⌘+Enter）
4. 見到 **Success. No rows returned** 就成功 ✅
...
2. 將以下檔案 **全部一次過** 揀晒上載（檔案要喺同一層，唔好放入資料夾）：
   ```
...
4. **Branch** 揀 **main** + **/(root)** → 撳 **Save**
5. 等約 1 分鐘，頁面頂會出現你嘅網址：
   ```
   https://你嘅github名.github.io/curryboy/
   ```
6. 用 Safari 開呢條網址，App 應該出到嚟 🎉

## 第 7 步：加到 iPhone 主畫面

1. 用 **Safari** 開上面條網址
2. 撳底部 **分享**（中間個方格加箭咀）
3. 揀 **加入主畫面（Add to Home Screen）**
4. 撳 **加入** → 主畫面就有 CurryBoy 圖示，撳一下全螢幕開，似真 App 一樣
